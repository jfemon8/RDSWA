    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {forms.map((f: any) => (
            <div key={f._id} className="border rounded-lg p-4 bg-background">
              ...
              {(f.status === 'pending' || f.status === 'under_review') && (
                <div className="flex gap-2">
                  <button onClick={() => reviewMutation.mutate({ id: f._id, status: 'approved' })}
                    className="flex items-center gap-1 px-3 py-1.5 text-xs bg-green-600 text-white rounded-md hover:bg-green-700">
                    <CheckCircle className="h-3 w-3" /> Approve
                  </button>
                  <button onClick={() => reviewMutation.mutate({ id: f._id, status: 'rejected', comment: 'Rejected by admin' })}
