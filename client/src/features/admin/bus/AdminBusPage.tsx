  ...
  return (
    <div className="flex gap-1.5">
      <motion.button
        ...
        onClick={async () => {
          try {
            const { data } = await api.get(`/bus/export/${type}?format=csv`, {
              responseType: "text",
            });
...
const ADMIN_BUS_TABS: readonly AdminBusTab[] = [
  "operators",
  "routes",
  "schedules",
  "counters",
    ...
    <FadeIn direction="up">
      <div className="container mx-auto">
        ...
        <div className="flex gap-1 sm:gap-2 mb-6 border-b overflow-x-auto no-scrollbar -mx-3 sm:mx-0 px-3 sm:px-0">
          {(
            ["operators", "routes", "schedules", "counters", "import"] as const
          ).map((t) => (
            <button
              key={t}
              onClick={() => setTab(t)}
...
function OperatorsList() {
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      editId
        ? api.patch(`/bus/operators/${editId}`, form)
        : api.post("/bus/operators", form),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bus", "operators"] });
      resetForm();
      toast.success(editId ? "Operator updated" : "Operator added");
    },
    onError: (err: any) => {
  ...
  const startEdit = (o: any) => {
    ...
    setForm({
      ...
      scheduleType: o.scheduleType || "both",
    });
    setShowForm(true);
  };
            ...
            <div className="border rounded-lg p-4 sm:p-6 bg-card mb-4">
              <form
                ...
                onSubmit={(e) => {
                  ...
                  setErrors({});
                  if (!form.name.trim()) {
                    setErrors({ name: "Operator name is required" });
                    return;
                  }
                  saveMutation.mutate();
                }}
              ...
              >
                ...
                <div>
                  ...
                  <RichTextEditor
                    ...
                    placeholder="Write about this operator..."
                    minHeight="100px"
                  />
                </div>
              ...
              <div className="border rounded-lg bg-card hover:bg-accent/30 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <motion.span
                      animate={{ rotate: expandedId === o._id ? 180 : 0 }}
                  ...
                  >
                    ...
                    <DeleteBtn
                      onClick={async () => {
                        ...
                        if (ok) deleteMutation.mutate(o._id);
                      }}
                    />
                  </div>
                </div>
                      ...
                      <div className="px-3 pb-3 pt-0 border-t mx-3 mb-3 space-y-2 text-xs">
                        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-2">
                          ...
                          {o.email && (
                            <div>
                              ...
                              <span className="text-foreground">{o.email}</span>
                            </div>
                          )}
                          {o.website && (
                            <div>
                        ...
                        {o.description && (
                          <div className="pt-2 border-t">
                            ...
                            <RichContent
                              html={o.description}
                              className="text-foreground prose-sm max-w-none"
                            />
                          </div>
                        )}
                      </div>
...
function RoutesList() {
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editId
        ? api.patch(`/bus/routes/${editId}`, payload)
        : api.post("/bus/routes", payload);
    },
    onSuccess: () => {
  ...
  const removeStop = (idx: number) =>
    setStops(
      stops.filter((_, i) => i !== idx).map((s, i) => ({ ...s, order: i })),
    );
  const updateStop = (idx: number, name: string) =>
    setStops(stops.map((s, i) => (i === idx ? { ...s, name } : s)));
              ...
              >
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setErrors((prev) => {
                          const { origin, ...rest } = prev;
                          return rest;
                        });
                      }}
                      className={`w-full px-3 py-2 border rounded-md bg-card text-foreground text-sm ${errors.origin ? "border-red-500" : ""}`}
                      required
                    />
                  ...
                  <div>
                    <input
                      ...
                      className={`w-full px-3 py-2 border rounded-md bg-card text-foreground text-sm ${errors.destination ? "border-red-500" : ""}`}
                      required
                    />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...form, routeType: e.target.value })
                    }
                    className="px-3 py-2 border rounded-md bg-card text-foreground text-sm"
                  >
                    <option value="university">University</option>
            ...
            >
              <div className="border rounded-lg bg-card hover:bg-accent/30 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <motion.span
                      animate={{ rotate: expandedId === r._id ? 180 : 0 }}
                      transition={{ duration: 0.2 }}
                    >
                      <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />
                    </motion.span>
                  ...
                  >
                    ...
                    <DeleteBtn
                      onClick={async () => {
                        const ok = await confirm({
                          title: "Delete Route",
                          message: `Delete route "${r.origin} → ${r.destination}"? This cannot be undone.`,
                          confirmLabel: "Delete",
                          variant: "danger",
                        });
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Select a route to manage schedules
        </p>
      </div>
      {routesLoading ? (
        <Spinner />
...
function RouteSchedules({ route, onBack }: { route: any; onBack: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    seasonalStartDate: "",
    seasonalEndDate: "",
    seasonalDepartureTime: "",
    seasonalArrivalTime: "",
    seasonalNote: "",
  });
  const emptyBus = (): ScheduleBus => ({
    operator: "",
    busName: "",
    busCategory: "non_ac",
  });
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/bus/schedules?route=${route._id}&limit=100`,
      );
      return data;
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      const payload: any = {
        ...
        buses: buses.filter((b) => b.operator),
      };
      if (seasonalSeason) {
      ...
      return editId
        ? api.patch(`/bus/schedules/${editId}`, payload)
        : api.post("/bus/schedules", payload);
    },
    onSuccess: () => {
  ...
  const startEdit = (s: any) => {
    ...
    setForm({
      ...
      seasonalStartDate: s.seasonalVariation?.startDate
        ? toDateInput(s.seasonalVariation.startDate)
        : "",
      seasonalEndDate: s.seasonalVariation?.endDate
        ? toDateInput(s.seasonalVariation.endDate)
        : "",
  ...
  const removeBus = (idx: number) =>
    setBuses(buses.length > 1 ? buses.filter((_, i) => i !== idx) : buses);
              ...
              <form
                ...
                onSubmit={(e) => {
                  ...
                  if (validBuses.length === 0)
                    errs.buses =
                      "At least one bus with an operator is required";
                  if (Object.keys(errs).length) {
                    setErrors(errs);
                    return;
                  }
              ...
              >
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setErrors((prev) => {
                            const { departureTime, ...rest } = prev;
                            return rest;
                          });
                        }}
                        className={`w-full px-3 py-2 border rounded-md bg-card text-foreground text-sm ${errors.departureTime ? "border-red-500" : ""}`}
                        required
                      />
                      ...
                      >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <select
                            value={bus.operator}
                            onChange={(e) =>
                              updateBus(idx, "operator", e.target.value)
                            }
                          ...
                          <select
                            value={bus.busCategory}
                            onChange={(e) =>
                              updateBus(idx, "busCategory", e.target.value)
                            }
                            className="px-3 py-1.5 border rounded-md bg-card text-foreground text-sm"
                          >
                        ...
                        <input
                          ...
                          value={bus.busName}
                          onChange={(e) =>
                            updateBus(idx, "busName", e.target.value)
                          }
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    type="checkbox"
                    checked={form.isSpecialSchedule}
                    onChange={(e) =>
                      setForm({ ...form, isSpecialSchedule: e.target.checked })
                    }
                    className="rounded cursor-pointer"
                  />
                  <span className="text-sm text-foreground">
                    Special/Seasonal Schedule
                  </span>
                </div>
                <AnimatePresence>
                  {form.isSpecialSchedule && (
                    ...
                    >
                      <input
                        placeholder="Note (e.g. Eid special, Winter schedule)"
                        value={form.specialScheduleNote}
                        onChange={(e) =>
                          setForm({
                            ...form,
                            specialScheduleNote: e.target.value,
                          })
                        }
                        className="w-full px-3 py-2 border rounded-md bg-card text-foreground text-sm"
                      />
                      <div className="border rounded-md p-3 bg-muted/30 space-y-3">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                          <div>
                            ...
                            <select
                              value={form.seasonalSeason}
                              onChange={(e) =>
                                setForm({
                                  ...form,
                                  seasonalSeason: e.target.value,
                                })
                              }
                              className="w-full px-3 py-1.5 border rounded-md bg-card text-foreground text-sm"
                            >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            ...
                            <input
                              type="time"
                              value={form.seasonalDepartureTime}
                              onChange={(e) =>
                                setForm({
                                  ...form,
                                  seasonalDepartureTime: e.target.value,
                                })
                              }
                              className="w-full px-3 py-1.5 border rounded-md bg-card text-foreground text-sm"
                            />
                          ...
                          <div>
                            ...
                            <input
                              type="time"
                              value={form.seasonalArrivalTime}
                              onChange={(e) =>
                                setForm({
                                  ...form,
                                  seasonalArrivalTime: e.target.value,
                                })
                              }
                              className="w-full px-3 py-1.5 border rounded-md bg-card text-foreground text-sm"
                            />
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm({ ...form, seasonalNote: e.target.value })
                          }
                          className="w-full px-3 py-1.5 border rounded-md bg-card text-foreground text-sm"
                        />
                      </div>
              ...
              <tbody>
                {schedules.map((s: any) => (
                  <tr key={s._id} className="border-t hover:bg-accent/30">
                    ...
                    <td className="p-3 text-foreground whitespace-nowrap">
                      {s.arrivalTime ? formatTimeString(s.arrivalTime) : "-"}
                    </td>
                      ...
                      {(s.buses || []).map((b: any, i: number) => (
                        <div key={i} className="py-0.5 break-words">
                          ...
                          <span className="capitalize text-muted-foreground">
                            {" "}
                            · {b.busCategory?.replace("_", " ")}
                          </span>
                        </div>
                      ))}
                      {!s.buses?.length && (
                        <span className="text-muted-foreground">No buses</span>
                      )}
                    </td>
                    <td className="p-3 capitalize text-xs text-muted-foreground break-words">
                      {s.daysOfOperation?.join(", ") || "Daily"}
                    </td>
                      ...
                      <div className="flex justify-end gap-1">
                        ...
                        <DeleteBtn
                          onClick={async () => {
                            ...
                            if (ok) deleteMutation.mutate(s._id);
                          }}
                        />
                      </div>
                    </td>
                  ...
                  <div>
                    <p className="text-sm font-medium text-foreground whitespace-nowrap">
                      ...
                      {s.arrivalTime && (
                        <span className="text-muted-foreground">
                          {" "}
                          → {formatTimeString(s.arrivalTime)}
                        </span>
                      )}
                    </p>
                    <p className="text-xs text-muted-foreground capitalize mt-0.5 break-words">
                      {s.daysOfOperation?.join(", ") || "Daily"}
                    </p>
                  </div>
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        ...
        <p className="text-sm text-muted-foreground">
          Select an operator to manage counters
        </p>
      </div>
      {operatorsLoading ? (
        <Spinner />
            ...
            >
              ...
              >
                ...
                <p className="text-xs text-muted-foreground">
                  <span className="capitalize">{o.scheduleType}</span>
                  {o.contactNumber ? ` · ${o.contactNumber}` : ""}
                </p>
              </motion.button>
            </FadeIn>
          ))}
...
function OperatorCounters({
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(`/bus/counters?operator=${operator._id}`);
      return data;
    },
  });
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editId
        ? api.patch(`/bus/counters/${editId}`, payload)
        : api.post("/bus/counters", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["bus", "counters"] });
      resetForm();
      toast.success(editId ? "Counter updated" : "Counter added");
    },
    onError: (err: any) => {
      const fe = extractFieldErrors(err);
      if (fe) {
        ...
        toast.error(err.response?.data?.message || "Failed to save counter");
      }
    },
  });
  ...
  const resetForm = () => {
    setShowForm(false);
    setEditId(null);
    setForm({ name: "", location: "", phoneNumbers: "", bookingLink: "" });
  };
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            <span className="capitalize">{operator.scheduleType}</span>
            {operator.contactNumber ? ` · ${operator.contactNumber}` : ""}
          </p>
        </div>
        <button
          onClick={() => {
            ...
            >
              <div className="border rounded-lg bg-card hover:bg-accent/30 transition-colors">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0">
                    <motion.span
                      animate={{ rotate: expandedId === c._id ? 180 : 0 }}
                      transition={{ duration: 0.2 }}
                    >
                      <ChevronDown className="h-3.5 w-3.5 text-muted-foreground" />
                    </motion.span>
                    ...
                    <DeleteBtn
                      onClick={async () => {
                        const ok = await confirm({
                          ...
                          message: `Delete counter "${c.name}"? This cannot be undone.`,
                          confirmLabel: "Delete",
                          variant: "danger",
                        });
    ...
    <FadeIn direction="up">
      <div>
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Paste a JSON array of records to import. Each object should match the
          expected fields for the selected type.
        </p>
              ...
              {result.error ? (
                ...
                <>
                  <p className="text-sm font-medium text-foreground">
                    {result.data?.created}/{result.data?.total} records imported
                  </p>
...
function EditBtn({ onClick }: { onClick: () => void }) {
  return (
    <button onClick={onClick} className="p-1.5 hover:bg-accent rounded">
...
function DeleteBtn({ onClick }: { onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="p-1.5 text-red-500 hover:bg-red-100 dark:hover:bg-red-950/30 rounded"
    >
