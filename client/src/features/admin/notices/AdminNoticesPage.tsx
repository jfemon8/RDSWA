...
export default function AdminNoticesPage() {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      const description =
        fieldErrors && Object.keys(fieldErrors).length > 1
              ...
              >
                ...
                <div>
                  <label className="text-xs text-muted-foreground font-medium block mb-1.5">
                    Attachment{" "}
                    <span className="opacity-70">(Image or PDF, max 5 MB)</span>
                  </label>
                  <input
                    ref={fileInputRef}
                    type="file"
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
                    ...
                    <div className="flex gap-1">
                        ...
                        <button
                          onClick={async () => {
                            const ok = await confirm({
                              ...
                              message: `Archive notice "${n.title}"? It will be removed from the active list.`,
                      ...
                      <button
                        onClick={async () => {
                          const ok = await confirm({
                            title: "Delete Notice",
                            message: `Delete notice "${n.title}"? This cannot be undone.`,
                            confirmLabel: "Delete",
                            variant: "danger",
                          });
          ...
          <InfiniteScrollSentinel
            ...
            endLabel={
              notices.length > 0 ? `All ${total} notices loaded` : undefined
            }
          />
        </div>
      )}
