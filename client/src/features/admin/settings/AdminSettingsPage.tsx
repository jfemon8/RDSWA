...
export default function AdminSettingsPage() {
  ...
  const effectiveTab: Tab =
    tab === "adsense" && !canManageAdsense ? "general" : tab;
    ...
    <FadeIn direction="up">
      <div className="container mx-auto">
        ...
        {effectiveTab === "organizations" && (
          <OrganizationsTab settings={settings} />
        )}
        ...
        {effectiveTab === "adsense" && canManageAdsense && (
...
function AdsenseTab({ settings: s }: { settings: any }) {
  ...
  const mutation = useMutation({
    mutationFn: (next: boolean) =>
      api.patch("/settings/adsense", { adsenseEnabled: next }),
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
      toast.success(
        variables
          ? "AdSense is now visible site-wide"
          : "AdSense is now hidden site-wide",
      );
    },
    onError: (err: any) =>
...
function GeneralTab({ settings: s }: { settings: any }) {
  ...
  useEffect(() => {
    setForm({
      siteName: s.siteName || "",
      siteNameFull: s.siteNameFull || "",
      siteNameBn: s.siteNameBn || "",
      siteNameBnFull: s.siteNameBnFull || "",
      ...
      footerLogoDark: s.footerLogoDark || "",
      favicon: s.favicon || "",
    });
  }, [s]);
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field
            ...
            placeholder="e.g. RDSWA"
          />
          <Field
            label="Full Name (EN)"
            value={form.siteNameFull}
            onChange={(v) => setForm({ ...form, siteNameFull: v })}
            placeholder="e.g. Rangpur Divisional Student Welfare Association"
          />
          <Field
            label="Short Name (বাংলা)"
            value={form.siteNameBn}
            onChange={(v) => setForm({ ...form, siteNameBn: v })}
            placeholder="e.g. রবিকস"
          />
          <Field
            label="Full Name (বাংলা)"
            value={form.siteNameBnFull}
            onChange={(v) => setForm({ ...form, siteNameBnFull: v })}
            placeholder="e.g. রংপুর বিভাগীয় ছাত্র কল্যাণ সমিতি"
          />
        </div>
        ...
        <Field
          ...
          onChange={(v) =>
            setForm({ ...form, foundedYear: parseInt(v) || 2021 })
          }
        />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <ImageUpload
              value={form.logoDark}
              onChange={(url) => setForm({ ...form, logoDark: url })}
              folder="branding"
            />
          </div>
          <div>
            ...
            <ImageUpload
              ...
              onChange={(url) => setForm({ ...form, footerLogo: url })}
              folder="branding"
            />
          </div>
          <div>
            ...
            <ImageUpload
              value={form.footerLogoDark}
              onChange={(url) => setForm({ ...form, footerLogoDark: url })}
              folder="branding"
            />
          </div>
          <div>
    ...
    <FadeIn direction="up" delay={0.1}>
      <div className="mt-10 pt-8 border-t space-y-4">
        ...
        <p className="text-sm text-muted-foreground -mt-2">
          These colors drive the primary / secondary buttons, links, tags, and
          accent surfaces across the whole website. Pick from the swatch or
          paste a 6-digit hex (e.g.{" "}
          <code className="px-1 rounded bg-muted">#008f57</code>). Empty values
          fall back to the brand defaults.
        </p>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2">
            ...
            {!allValid && (
              <span className="text-xs text-destructive">
                One or more values aren't valid 6-digit hex codes.
              </span>
            )}
          </div>
        )}
...
function HomepageTab({ settings: s }: { settings: any }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      heroTitleGradient: h.heroTitleGradient || "",
      heroBadgeText: h.heroBadgeText || "",
      heroSubtitle: h.heroSubtitle || "",
      heroImage: h.heroImage || "",
      heroTagline: h.heroTagline || "",
      rotatingWords: h.rotatingWords || [],
      introText: h.introText || "",
      featuresHeading: h.featuresHeading || "",
      featuresSubheading: h.featuresSubheading || "",
      features: h.features || [],
      servicesHeading: h.servicesHeading || "",
      services: h.services || [],
      ctaTitle: h.ctaTitle || "",
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch("/settings/homepage", { homePageContent: form }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
    ...
    <FadeIn direction="up">
      <div className="space-y-4">
        ...
        <Field
          ...
          onChange={(v) => setForm({ ...form, heroBadgeText: v })}
        />
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field
          ...
          />
          <Field
            label="Hero Title Gradient (Line 2)"
            value={form.heroTitleGradient}
            onChange={(v) => setForm({ ...form, heroTitleGradient: v })}
          />
        </div>
        <Field
          label="Hero Subtitle"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <Field
            label="Tagline (before rotating words)"
            value={form.heroTagline}
            onChange={(v) => setForm({ ...form, heroTagline: v })}
          />
          <Field
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <Field
            ...
            onChange={(v) => setForm({ ...form, ctaButtonText: v })}
          />
        </div>
        <div>
        ...
        <Field
          ...
          onChange={(v) => setForm({ ...form, featuresHeading: v })}
        />
        <Field
          label="Features Subheading"
          value={form.featuresSubheading}
          onChange={(v) => setForm({ ...form, featuresSubheading: v })}
        />
        <ArrayEditor
          items={form.features}
        ...
        <Field
          ...
          onChange={(v) => setForm({ ...form, servicesHeading: v })}
        />
        <ArrayEditor
          items={form.services}
    ...
    <FadeIn direction="up">
      <div className="space-y-4">
        <RichField
          ...
          onChange={(v) => setForm({ ...form, aboutContent: v })}
          placeholder="About the association..."
        />
        <RichField
          label="Mission"
          value={form.missionContent}
          onChange={(v) => setForm({ ...form, missionContent: v })}
          placeholder="Mission statement..."
        />
        <RichField
          label="Vision"
          value={form.visionContent}
          onChange={(v) => setForm({ ...form, visionContent: v })}
          placeholder="Vision statement..."
        />
        <RichField
          label="Objectives"
          value={form.objectivesContent}
          onChange={(v) => setForm({ ...form, objectivesContent: v })}
          placeholder="Objectives..."
        />
        <RichField
          label="History"
          value={form.historyContent}
          onChange={(v) => setForm({ ...form, historyContent: v })}
...
function UniversityTab({ settings: s }: { settings: any }) {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch("/settings/university", { universityInfo: form }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
      toast.success("University info saved");
    },
    onError: (err: any) =>
      toast.error(err.response?.data?.message || "Failed to save"),
  });
    ...
    <FadeIn direction="up">
      <div className="space-y-4">
        <h2 className="text-lg font-semibold text-foreground flex items-center gap-2">
          <GraduationCap className="h-5 w-5" /> University Details
        </h2>
        <Field
...
function OrganizationsTab({ settings: s }: { settings: any }) {
  ...
  useEffect(() => {
    setOrgs(
      (s.otherOrganizations || []).map((o: any) => ({
        name: o.name || "",
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch("/settings/organizations", { otherOrganizations: orgs }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
      ...
      <div className="space-y-4">
        ...
        {orgs.map((org, i) => (
          ...
          >
            <button
              type="button"
              onClick={() => setOrgs(orgs.filter((_, idx) => idx !== i))}
            ...
            <input
              ...
              onChange={(e) => {
                const o = [...orgs];
                o[i] = { ...o[i], website: e.target.value };
                setOrgs(o);
              }}
              className="w-full px-3 py-2 border rounded-md bg-card text-foreground text-sm"
            />
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          {faq.map((item, i) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 10 }}
              animate={{ opacity: 1, y: 0 }}
              className="border rounded-lg p-3 space-y-2 relative mb-2"
            >
              <button
                type="button"
          ...
          {faq.length === 0 && (
            <p className="text-sm text-muted-foreground">No FAQ items.</p>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              onClick={() =>
                setPrivacy([...privacy, { title: "", content: "" }])
              }
              className="flex items-center gap-1 px-3 py-1 text-xs bg-primary/10 text-primary rounded-md hover:bg-primary/20"
            >
          ...
          {privacy.map((item, i) => (
            ...
            >
              <button
                type="button"
                onClick={() =>
                  setPrivacy(privacy.filter((_, idx) => idx !== i))
                }
                className="absolute top-2 right-2 text-muted-foreground hover:text-red-500"
              >
                <Trash2 className="h-3.5 w-3.5" />
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              type="button"
              onClick={() => setTerms([...terms, { title: "", content: "" }])}
              className="flex items-center gap-1 px-3 py-1 text-xs bg-primary/10 text-primary rounded-md hover:bg-primary/20"
            >
              <Plus className="h-3 w-3" /> Add Section
            </button>
...
function SocialTab({ settings: s }: { settings: any }) {
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      twitter: sl.twitter || "",
      androidApp: sl.androidApp || "",
      iosApp: sl.iosApp || "",
      windowsApp: sl.windowsApp || "",
      macosApp: sl.macosApp || "",
    ...
    <FadeIn direction="up">
      <div className="space-y-4">
        ...
        <Field
          ...
          onChange={(v) => setForm({ ...form, androidApp: v })}
          placeholder="https://play.google.com/store/apps/details?id=..."
        />
        <Field
          label="iOS App Link"
          value={form.iosApp}
          onChange={(v) => setForm({ ...form, iosApp: v })}
          placeholder="https://apps.apple.com/app/id..."
        />
        <Field
          label="Windows App Link"
          value={form.windowsApp}
          onChange={(v) => setForm({ ...form, windowsApp: v })}
          placeholder="https://apps.microsoft.com/detail/..."
        />
        <Field
          label="macOS App Link"
          value={form.macosApp}
          onChange={(v) => setForm({ ...form, macosApp: v })}
          placeholder="https://apps.apple.com/app/mac/id..."
        />
        <Field
          label="Linux App Link"
          value={form.linuxApp}
          onChange={(v) => setForm({ ...form, linuxApp: v })}
          placeholder="https://snapcraft.io/... or https://flathub.org/apps/..."
        />
...
function AcademicConfigSection() {
  ...
  const [faculties, setFaculties] = useState<
    Array<{ name: string; departments: string[] }>
  >([]);
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch("/settings/academic-config", {
        batches: batches
          .split(",")
          .map((b) => b.trim())
          .filter(Boolean),
  ...
  return (
    <FadeIn direction="up" delay={0.1}>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Manage dropdown values for batch, session, faculty, and department used
        in user profiles.
      </p>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button
              type="button"
              onClick={() =>
                setFaculties([...faculties, { name: "", departments: [] }])
              }
              className="flex items-center gap-1 px-3 py-1 text-xs bg-primary/10 text-primary rounded-md hover:bg-primary/20"
            >
            ...
            {faculties.map((fac, i) => (
              ...
              >
                <button
                  type="button"
                  onClick={() =>
                    setFaculties(faculties.filter((_, idx) => idx !== i))
                  }
          ...
          {faculties.length === 0 && (
            <p className="text-sm text-muted-foreground">No faculties.</p>
          )}
        </div>
