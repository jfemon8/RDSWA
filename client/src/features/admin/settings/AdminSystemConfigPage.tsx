    ...
    <FadeIn direction="up" delay={0.1}>
      <div className="border rounded-lg p-5 bg-card space-y-4 ">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Min Options per Vote
            </label>
            <input
              type="number"
              value={form.minOptions}
              onChange={(e) =>
                setForm({ ...form, minOptions: parseInt(e.target.value) || 2 })
              }
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Max Options per Vote
            </label>
            <input
              type="number"
              value={form.maxOptions}
              onChange={(e) =>
                setForm({ ...form, maxOptions: parseInt(e.target.value) || 10 })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={2}
            />
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Min Vote Duration (hours)
            </label>
            <input
              type="number"
              value={form.minVoteDurationHours}
              onChange={(e) =>
                setForm({
                  ...form,
                  minVoteDurationHours: parseInt(e.target.value) || 1,
                })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={1}
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Max Vote Duration (days)
            </label>
            <input
              type="number"
              value={form.maxVoteDurationDays}
              onChange={(e) =>
                setForm({
                  ...form,
                  maxVoteDurationDays: parseInt(e.target.value) || 30,
                })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={1}
          ...
          <div>
            ...
            <input
              type="number"
              value={form.requireMinVoterPercent}
              onChange={(e) =>
                setForm({
                  ...form,
                  requireMinVoterPercent: parseInt(e.target.value) || 0,
                })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={0}
          ...
          <div>
            ...
            <select
              value={form.defaultEligibility}
              onChange={(e) =>
                setForm({ ...form, defaultEligibility: e.target.value })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
            >
        ...
        <label className="flex items-center gap-2 text-sm">
          <input
            type="checkbox"
            checked={form.allowAnonymousResults}
            onChange={(e) =>
              setForm({ ...form, allowAnonymousResults: e.target.checked })
            }
            className="rounded"
          />
...
function MembershipCriteriaConfig() {
  ...
  const [form, setForm] = useState<MembershipCriteria>(
    DEFAULT_MEMBERSHIP_CRITERIA,
  );
  ...
  useEffect(() => {
    ...
    setForm({
      ...
      ...data,
      academicDocs: {
        ...DEFAULT_MEMBERSHIP_CRITERIA.academicDocs,
        ...(data.academicDocs || {}),
      },
      identityDocs: {
        ...DEFAULT_MEMBERSHIP_CRITERIA.identityDocs,
        ...(data.identityDocs || {}),
      },
    });
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["settings"] });
      toast.success("Membership criteria updated");
    },
    onError: (err: any) => {
      toast.error(err.response?.data?.message || "Failed");
    },
  ...
  const toggleAccepted = (
    group: "academicDocs" | "identityDocs",
    key: string,
  ) => {
    ...
    <FadeIn direction="up" delay={0.1}>
      <div className="border rounded-lg p-5 bg-card space-y-5 ">
        ...
        <div className="space-y-2">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, requirePhoneVerification: e.target.checked })
              }
              className="rounded"
            />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Allowed Divisions (comma separated)
            </label>
            <input
              value={form.allowedDivisions.join(", ")}
              onChange={(e) =>
                setForm({
                  ...form,
                  allowedDivisions: e.target.value
                    .split(",")
                    .map((s) => s.trim())
                    .filter(Boolean),
                })
          ...
          <div>
            ...
            <input
              ...
              value={form.minBatch}
              onChange={(e) =>
                setForm({ ...form, minBatch: parseInt(e.target.value) || 1 })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={1}
            />
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
              ...
              <span
                title="SLA warning threshold. Pending forms older than this show an 'overdue' badge in the review queue. No automated action is taken."
            ...
            <input
              type="number"
              value={form.maxPendingDays}
              onChange={(e) =>
                setForm({
                  ...form,
                  maxPendingDays: parseInt(e.target.value) || 7,
                })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={1}
          ...
          <div>
            <label className="text-xs text-muted-foreground mb-1 block">
              Auto-Reject After Days (0 = disabled)
            </label>
            <input
              type="number"
              ...
              onChange={(e) =>
                setForm({
                  ...form,
                  autoRejectAfterDays: parseInt(e.target.value) || 0,
                })
              }
              className="w-full px-3 py-2 border rounded-md bg-background text-sm"
              min={0}
...
function AutoRoleConfig() {
  ...
  const { data: liveConfig, isLoading, isError, refetch } = useAutoRoleConfig();

  // Only for the read-only summaries above the editor, which are harmless when the load is still in flight.
  const cfg = liveConfig ?? AUTO_ROLE_CONFIG_FALLBACK;

  const tierRoleDescriptions: Record<string, string> = {
    [UserRole.GUEST]: "Unauthenticated visitor",
    [UserRole.USER]: "Registered account, membership not approved yet",
    [UserRole.MEMBER]: "Approved member",
    [UserRole.MODERATOR]: "Content moderator",
    [UserRole.ADMIN]: "Administrator",
    [UserRole.SUPER_ADMIN]: "Highest tier",
  };

  const tagDescriptions: Record<string, { label: string; desc: string }> = {
    [UserRole.ALUMNI]: {
      label: "Alumni",
      desc: "Approved member with current job/business or admin grant",
    },
    [UserRole.ADVISOR]: {
      label: "Advisor",
      ...
      <div className="space-y-5">
        <div className="border rounded-lg p-5 bg-card">
          ...
          <div className="flex items-start gap-2 p-3 rounded-md bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-800/40 text-xs text-amber-800 dark:text-amber-300">
            ...
            <p>
              The position lists below drive both automatic role transitions
              (committee create / member add / archive / startup sync) and the
              role-change validation. Defaults match the original RDSWA
              constitution.
            </p>
          </div>
        </div>
        ...
        <div className="border rounded-lg p-5 bg-card space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-foreground text-sm">
              Tier Roles (single privilege level)
            </h4>
          </div>
          <p className="text-xs text-muted-foreground">
            A user has exactly one tier role. Higher tier inherits all
            permissions below.
          </p>
        ...
        <div className="border rounded-lg p-5 bg-card space-y-3">
          ...
          <p className="text-xs text-muted-foreground">
            Independent of tier role. A user may carry any combination of these
            tags alongside any tier role. Granted via dedicated endpoints, not
            via the role-change dropdown.
          </p>
              ...
              <div key={r} className="rounded-md border p-3 bg-background/50">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium text-sm">
                    {tagDescriptions[r].label}
                  </span>
                </div>
                <p className="text-[11px] text-muted-foreground">
                  {tagDescriptions[r].desc}
                </p>
              </div>
            ))}
          </div>
        ...
        <div className="border rounded-lg p-5 bg-card space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-foreground text-sm">
              Auto-Role from Current Committee Position
            </h4>
          </div>
          <p className="text-xs text-muted-foreground">
        ...
        <div className="border rounded-lg p-5 bg-card space-y-3">
          <div className="flex items-center gap-2">
            ...
            <h4 className="font-semibold text-foreground text-sm">
              Archive Transitions (committee marked historical)
            </h4>
          </div>
          <p className="text-xs text-muted-foreground">
            When a committee transitions from current to archived, ex-officers
            transition based on their position.
          </p>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 flex-wrap text-sm">
              <div className="flex flex-wrap gap-1.5 min-w-[6rem]">
                {cfg.adminPositions.length > 0 ? (
                  cfg.adminPositions.map((p) => (
                    <span
                      key={p}
                      className="px-2 py-0.5 rounded-md bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 text-xs capitalize"
                    >
                      ex-{formatPos(p)}
                    </span>
                  ))
              ...
              <span className="font-mono text-xs px-2 py-0.5 rounded bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
                admin → moderator
              </span>
            </div>
            <div className="flex items-center gap-2 flex-wrap text-sm">
              ...
              <ArrowRight className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
              <span className="font-mono text-xs px-2 py-0.5 rounded bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400">
                moderator → base (member/user)
              </span>
            </div>
            <div className="flex items-center gap-2 flex-wrap text-sm">
              <div className="flex flex-wrap gap-1.5 min-w-[6rem]">
              ...
              <ArrowRight className="h-3.5 w-3.5 text-muted-foreground shrink-0" />
              <span className="font-mono text-xs px-2 py-0.5 rounded bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400">
                + advisor tag
              </span>
            </div>
          </div>
          ...
          <p className="text-[11px] text-muted-foreground pt-2 border-t">
            Multi-committee safeguard: a user is only downgraded if they don't
            still hold a qualifying position in another current committee.
          </p>
        </div>
        ...
        <div className="border rounded-lg p-5 bg-card space-y-3">
          ...
          <ul className="text-xs space-y-1.5 text-muted-foreground list-disc list-inside">
            <li>
              Tier-role dropdown only exposes{" "}
              <span className="font-medium text-foreground">
                user / member / moderator / admin
              </span>
              . Tag roles are not selectable as tiers.
            </li>
            <li>
              Only{" "}
              <span className="font-medium text-foreground">SuperAdmin</span>{" "}
              can manually assign the <span className="font-mono">admin</span>{" "}
              role.
            </li>
            <li>
              SuperAdmin emails are hardcoded ({SUPER_ADMIN_EMAILS.length}{" "}
              accounts) and cannot be demoted.
            </li>
            <li>
            ...
            <li>
              ...
              auto-approves membership; demoting below member reverts it.
            </li>
          </ul>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="border-2 border-destructive/40 rounded-lg p-5 bg-destructive/5 text-sm text-foreground">
            <p className="font-medium mb-1">Could not load the current rules</p>
            <p className="text-xs text-muted-foreground mb-3">
              Editing is disabled until they load, so a save cannot overwrite them with defaults.
            </p>
            <button
              onClick={() => refetch()}
              className="px-3 py-1.5 border rounded-md text-xs hover:bg-accent"
            >
              Try again
            </button>
          </div>
        ) : (
          <AutoRoleEditor liveConfig={liveConfig} />
        )}
      </div>
    </FadeIn>
  );
...
function AutoRoleEditor({ liveConfig }: { liveConfig: AutoRoleConfigShape }) {
  ...
  const [adminPositions, setAdminPositions] = useState<Set<string>>(
    new Set(liveConfig.adminPositions),
  );
  const [moderatorPositions, setModeratorPositions] = useState<Set<string>>(
    new Set(liveConfig.moderatorPositions),
  );
  const [advisorOnArchivePositions, setAdvisorOnArchivePositions] = useState<
    Set<string>
  >(new Set(liveConfig.advisorOnArchivePositions));
  ...
  const dirty =
    ...
    setEqualsArray(moderatorPositions, liveConfig.moderatorPositions) ===
      false ||
    setEqualsArray(
      advisorOnArchivePositions,
      liveConfig.advisorOnArchivePositions,
    ) === false;
  ...
  return (
    <div className="border-2 border-primary/30 rounded-lg p-5 bg-primary/5 space-y-5">
      ...
      <p className="text-xs text-muted-foreground">
        Changes apply to all future committee transitions and the next
        server-startup role sync. They do not retroactively change existing user
        roles.
      </p>
      ...
      <div className="space-y-3">
        <div>
          ...
          {renderChips(adminPositions, setAdminPositions, {
            set: moderatorPositions,
            setFn: setModeratorPositions,
          })}
        </div>
        ...
        <div>
          ...
          {renderChips(moderatorPositions, setModeratorPositions, {
            set: adminPositions,
        ...
        <div>
          ...
          {renderChips(
            advisorOnArchivePositions,
            setAdvisorOnArchivePositions,
            undefined,
            "bg-emerald-100 dark:bg-emerald-900/30 border-emerald-300 dark:border-emerald-800 text-emerald-700 dark:text-emerald-400",
          )}
        </div>
      </div>
...
function MentorshipConfig() {
  ...
  const mutation = useMutation({
    mutationFn: () =>
      api.patch("/settings/mentorship-config", {
        areas,
  ...
  return (
    <div className="space-y-4">
      <div className="border rounded-lg p-5 bg-card space-y-3">
        ...
        <p className="text-xs text-muted-foreground">
          Mentors pick from this list and mentees request against it, which is
          what lets the two sides match.
        </p>
        <div className="flex flex-wrap gap-1.5">
          ...
          {areas.map((a) => (
            <span
              key={a}
              className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-muted text-xs text-foreground"
            >
              {a}
              <button
                type="button"
        ...
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={addArea}
            className="px-3 py-2 border rounded-md text-sm hover:bg-accent text-foreground"
          >
            Add
          </button>
        </div>
      ...
      <div className="border rounded-lg p-5 bg-card grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          />
          <p className="text-[11px] text-muted-foreground mt-1">
            Zero removes the limit entirely.
          </p>
        </div>
        <div>
          <label className="text-xs font-medium text-muted-foreground mb-1 block">
            Stale request reminder (days)
          </label>
          <input
            type="number"
            min="0"
            value={staleRequestDays}
            onChange={(e) => setStaleRequestDays(e.target.value)}
            className="w-full px-3 py-2 border rounded-md bg-background text-foreground text-sm"
          />
          <p className="text-[11px] text-muted-foreground mt-1">
            A mentor is reminded once per unanswered request; zero turns
            reminders off.
          </p>
        </div>
      </div>
