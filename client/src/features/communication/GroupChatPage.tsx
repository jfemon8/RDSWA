...
export default function GroupChatPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/communication/groups/${id}?limit=${50 * pagesLoaded}`,
      );
      return data.data;
    },
    enabled: !!id,
  ...
  const editMutation = useMutation({
    mutationFn: ({
      ...
      api.patch(`/communication/groups/${id}/messages/${messageId}`, {
        content,
      }),
    onSuccess: () => {
      setEditingId(null);
  ...
  const reactMutation = useMutation({
    mutationFn: ({ messageId, emoji }: { messageId: string; emoji: string }) =>
      api.post(`/communication/groups/${id}/messages/${messageId}/react`, {
        emoji,
      }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["group", id] }),
    onError: (err: any) =>
  ...
  const starMutation = useMutation({
    mutationFn: (messageId: string) =>
      api.post(`/communication/messages/${messageId}/star`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["group", id] }),
  });
  ...
  const muteMutation = useMutation({
    mutationFn: (mute: boolean) =>
      api.patch(`/communication/groups/${id}/mute`, { mute }),
    onSuccess: (_res, mute) => {
      queryClient.invalidateQueries({ queryKey: ["group", id] });
  ...
  const { data: joinRequests } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/communication/groups/${id}/join-requests`,
      );
      return data.data;
    },
  ...
  const reviewJoinRequestMutation = useMutation({
    mutationFn: (vars: {
      ...
      api.patch(`/communication/groups/${id}/join-requests/${vars.requestId}`, {
        status: vars.status,
      }),
    onSuccess: (_d, vars) => {
  ...
  const { data: searchResults } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/users/members?search=${encodeURIComponent(memberSearch)}&limit=10`,
      );
      return data.data;
    },
  ...
  const addMemberMutation = useMutation({
    mutationFn: (userId: string) =>
      api.post(`/communication/groups/${id}/members`, { userId }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["group", id] });
  ...
  const removeMemberMutation = useMutation({
    mutationFn: (userId: string) =>
      api.delete(`/communication/groups/${id}/members/${userId}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["group", id] });
  ...
  const { data: searchMessages, isLoading: searchLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get(
        `/communication/groups/${id}/search?q=${encodeURIComponent(searchQuery)}`,
      );
      return data.data;
    },
  ...
  const handleDeleteEveryone = async (messageId: string) => {
    const ok = await confirm({
      ...
      message:
        "This message will be removed for all participants. Attached files will also be deleted from storage.",
  ...
  const handleVisibleMessages = useCallback(
    (messageIds: string[]) => {
      ...
      const toMark = messageIds.filter((mid) => {
        ...
        const alreadyRead = (msg as any).readBy?.some(
          (r: any) => (r.user?._id || r.user) === user?._id,
        );
        return !alreadyRead;
      });
            ...
            {menuOpen && (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => {
                    setMenuOpen(false);
                    muteMutation.mutate(!group.isMuted);
                  }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-accent transition-colors"
                >
                  {group.isMuted ? (
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <MessageList
            ...
            onSubmitEdit={(mid, content) =>
              editMutation.mutate({ messageId: mid, content })
            }
          />
          <ChatComposer
            onSend={handleSend}
                            ...
                            {searchResults
                              ...
                              .map((u: any) => (
                                <button
                                  key={u._id}
                                  onClick={() =>
                                    addMemberMutation.mutate(u._id)
                                  }
                    ...
                    (joinRequests || []).length > 0 && (
                      <div className="mb-3">
                        <p className="text-[11px] font-semibold text-amber-600 dark:text-amber-400 mb-1.5 flex items-center gap-1">
                          <Clock className="h-3 w-3" /> Pending join requests (
                          {joinRequests.length})
                        </p>
                                ...
                                <button
                                  onClick={async () => {
                                    const ok = await confirm({
                                      ...
                                      message: `Reject ${req.user?.name || "this user"}'s request to join the group?`,
                                      confirmLabel: "Reject",
                                      variant: "danger",
                                    });
                                    if (ok)
